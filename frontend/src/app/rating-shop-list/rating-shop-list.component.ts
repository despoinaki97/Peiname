...
export class RatingShopListComponent implements OnInit {
  users: ordAccount[];
  Shops;
  constructor(private databank:DatabankService,private _leapService:LeapService,private socket:SocketsService,private router:Router) {
  ...
  var sub = this._leapService.gestureRecognizer().subscribe((gesture)=>{
    if(gesture == Gestures.SWIPE_RIGHT){
      ...
      document.getElementById('horscroll').scrollTo(-1500, 0);
